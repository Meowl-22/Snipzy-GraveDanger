...
if (isOpen) {
	...
	if (!surface_exists(shellSurface)) {
		...
		// =========================================================================
		// --- [MODIFICATION] FORCED COMPACT DIMENSIONS ---
		// =========================================================================
		width         = 500;  // Adjust this to change how narrow/wide the menu is
		height        = 220;  // Adjust this to change how short/tall the menu is
		visibleWidth  = width;
		visibleHeight = height - (consolePaddingV * 2) - emHeight;
		// =========================================================================
		
		// Force bottom-center positioning on initial setup
		shellOriginX = (display_get_gui_width() - width) / 2;
		shellOriginY = display_get_gui_height() - height;
	...
	// =========================================================================
	if (!variable_instance_exists(id, "isDragging")) {
		isDragging = false;
		dragOffsetX = 0;
		dragOffsetY = 0;
	}
	...
	if (mouse_check_button_pressed(mb_left)) {
		// Only start dragging if clicking inside the main console window
		if (point_in_rectangle(mx, my, shellOriginX, shellOriginY, shellOriginX + width, shellOriginY + height)) {
			// Prevent drag if clicking an autocomplete mouse selection
			if (!variable_instance_exists(id, "isAutocompleteOpen") || !isAutocompleteOpen) {
				isDragging = true;
	...
	if (isDragging) {
		if (mouse_check_button(mb_left)) {
			shellOriginX = mx - dragOffsetX;
			shellOriginY = my - dragOffsetY;
			
			// Clamp to screen boundaries so the player can't drag it entirely off-screen
			shellOriginX = clamp(shellOriginX, 0, display_get_gui_width() - width);
			shellOriginY = clamp(shellOriginY, 0, display_get_gui_height() - height);
		} else {
			isDragging = false;
		}
	}
	...
	surface_set_target(shellSurface);
		...
		if (consoleBackground != noone) {
			...
			draw_roundrect_ext(shellOriginX - 2, shellOriginY + 2, shellOriginX + width + 2, shellOriginY + height + 4, 10, 10, false);

			// Fancy Main Window Pane
			draw_set_alpha(consoleAlpha);
			draw_set_color(consoleColor);
			draw_roundrect_ext(shellOriginX, shellOriginY, shellOriginX + width, shellOriginY + height, 8, 8, false);
			
			// Sleek top accent line banner
			draw_set_alpha(min(1.0, consoleAlpha + 0.2));
			draw_set_color(promptColor);
			draw_line_width(shellOriginX + 4, shellOriginY + 1, shellOriginX + width - 4, shellOriginY + 1, 2);
		}
		...
		if (outputHeight > visibleHeight - emHeight) {
			...
			draw_set_alpha(0.15);
			draw_set_color(fontColorSecondary);
			draw_roundrect_ext(x1, y1, x2, y2, 4, 4, false);
		...
		if (array_length(filteredSuggestions) > 0) {
			if (enableAutocomplete && autocompleteMaxLines > 0) {
				...
				if (suggestionsAmount < array_length(filteredSuggestions)) {
					draw_set_alpha(0.2);
					draw_roundrect_ext(x2 - scrollbarWidth, y1 + 2, x2 - 2, y2 - 2, 4, 4, false);
					...
					draw_set_alpha(0.8);
					draw_set_color(fontColor);
					draw_roundrect_ext(x2 - scrollbarWidth, yScroll1, x2 - 2, yScroll2, 4, 4, false);
				}
