// --- System Configuration ---
create_image = false;
pause = false;
pause_alpha = 0;
pause_image = spr_null;
optionselected = 0;
inputbuffer = 0;
angel_timer = 240;

// --- Palette & Pattern Cache Hooks ---
saved_pal_index = 1;     // Temporarily holds the player's palette ID
saved_pattern_spr = noone; // Temporarily holds the player's texture pattern

// --- Visual & State Management ---
menu_dark = true;
painless = false;
image_speed = 0.35;
cur_selected = 1;
cur_anim_num = 1;

// --- Enum Structural Simulation ---
optiontypes = {
    hub: 0,
    level: 1,
    both: 2
};

// --- Constructors ---
create_pause_option = function(_name = "", _type = optiontypes.both, _icon_index = 0, _func = undefined) constructor
{
    o_name = _name;
    o_type = _type;
    o_func = _func;
    icon_index = _icon_index;
    iconalpha = 0;
}

create_pause_screen_asset = function(_spr, _startx = 0, _starty = 0, _endx = 0, _endy = 0, _xscale = 1, _yscale = 1) constructor
{
    sprite_index = _spr;
    startx = _startx;
    starty = _starty;
    endx = _endx;
    endy = _endy;
    x = startx;
    y = starty;
    image_xscale = _xscale;
    image_yscale = _yscale;
}

// --- Menu Options Definition ---
baseoptions = [
    new create_pause_option("RESUME",           optiontypes.both,  0),
    new create_pause_option("OPTIONS",          optiontypes.both,  1, function() {
        instance_create(0, 0, obj_options);
        scr_sound(choose(sfx_ui_accept1, sfx_ui_accept2, sfx_ui_accept3));
    }),
    new create_pause_option("MAIN MENU",        optiontypes.hub,   3, function() {
        do_unpause();
        if (obj_music.mu != noone) audio_stop_sound(obj_music.mu);
        if (obj_music.secret_mu != noone) audio_stop_sound(obj_music.secret_mu);
        room_goto(mainmenu);
        with (obj_player)
        {
            reset_anim(spr_player_entergate);
            spawn = "a";
            state = states.actor;
            door_type = fade_types.door;
            hsp = 0;
            vsp = 0;
        }
        reset_level();
    }),
    new create_pause_option("RESTART LEVEL",    optiontypes.level, 2, function() {
        do_unpause();
        if (obj_music.mu != noone) audio_stop_sound(obj_music.mu);
        if (obj_music.secret_mu != noone) audio_stop_sound(obj_music.secret_mu);
        if (global.start_room != noone)
        {
            room_goto(global.start_room);
            with (obj_player)
            {
                reset_anim(spr_player_entergate);
                spawn = "a";
                state = states.actor;
                door_type = fade_types.door;
                hsp = 0;
                vsp = 0;
                xscale = 1;
            }
        }
        else show_debug_message("start room was unset");
        reset_level();
    }),
    new create_pause_option("CHEF TASKS",        optiontypes.level, 8),
    new create_pause_option("EXIT LEVEL",        optiontypes.level, 3, function() {
        do_unpause();
        if (obj_music.mu != noone) audio_stop_sound(obj_music.mu);
        if (obj_music.secret_mu != noone) audio_stop_sound(obj_music.secret_mu);
        with (obj_player)
        {
            spawn = noone;
            state = states.backtohub;
            vsp = 0;
            sprite_index = spr_player_slipbanana1;
            x = return_location.x;
            y = return_location.y;
            room_goto(return_location.room);
        }
        instance_create(0, 0, obj_fadevisual);
        reset_level();
        global.in_level = false;
        global.doorshut = true;
    })
];

options = [];

cursor = {
    x: 0,
    y: 0,
    sprite_index: spr_pause_arrow,
    image_index: 0,
    image_speed: 0.35
};

screen_assets = [
    new create_pause_screen_asset(spr_pause_border, -160, screen_h + 188, 0, screen_h),
    new create_pause_screen_asset(spr_pause_border, screen_w + 160, screen_h + 188, screen_w, screen_h, -1),
    new create_pause_screen_asset(spr_pause_vines, screen_w / 2, -177, screen_w / 2, 0)
];

// Audio Initialization Weights
audio_master_gain(global.option_master_volume);
audio_group_set_gain(ag_sfx, global.option_sfx_volume, 0);
audio_group_set_gain(ag_music, global.option_music_volume, 0);