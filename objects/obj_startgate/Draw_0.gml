...
if surface_exists(bg_surf)
{
    ...
    var surface_floor = sprite_get_height(door_gate) - sprite_get_height(door_bg);
    
    for (var i = 0; i < array_length(bg_parallax); i++) //draw each layer
    {
        // Anchors the background image to the bottom floor line instead of floating
        draw_sprite_tiled(door_bg, i, bg_parallax[i].x, surface_floor + bg_parallax[i].y)
    }
