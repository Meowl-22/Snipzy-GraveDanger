...
// --- NEW PIZZAFACE CHECK ---
// If Pizzaface is in the same room, override all other music logic
if (instance_exists(obj_pizzaface))
{
	if (panic_mu == noone || audio_sound_get_asset(panic_mu) != mu_DeadBrotherChase)
	{
		// Stop any currently playing music tracks
		if (mu != noone) { audio_stop_sound(mu); mu = noone; }
		if (panic_mu != noone) { audio_stop_sound(panic_mu); panic_mu = noone; }
		if (panic_pinch_mu != noone) { audio_stop_sound(panic_pinch_mu); panic_pinch_mu = noone; }
		if (prevmu != noone) { audio_stop_sound(prevmu); prevmu = noone; }
		if (pillar_mu != noone) { audio_stop_sound(pillar_mu); pillar_mu = noone; }
		
		// Play Dead Brother Chase and loop it
		panic_mu = scr_sound(mu_DeadBrotherChase, true);
	}
}
else // Run the normal music logic if Pizzaface is not in the room
{
	...
	// New Check: See if the player is actively entering a gate right now
	var entering_gate = false;
	if (instance_exists(obj_player) && obj_player.sprite_index == spr_player_entergate)
	{
		entering_gate = true;
	}
	...
	if !isPanic
	{
		// FIXED: Added "&& !entering_gate" so it stops fighting your gate fade!
		if (instance_exists(obj_pillar) && !entering_gate)
		{
			if pillar_visible
			{
				if mu != noone
